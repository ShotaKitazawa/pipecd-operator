...
func NewPipeCDWebService(
	...
	c client.Client,
	namespace string,
	encryptionKeyRef pipecdv1alpha1.EmbeddedSecretKeyRef,
	projectId string,
	insecure bool,
	...
	if err := c.Get(ctx, types.NamespacedName{Namespace: namespace, Name: encryptionKeyRef.SecretName}, &secret); err != nil {
		return nil, ctx, err
	}
	encryptionKeyBytes, ok := secret.Data[encryptionKeyRef.Key]
	if !ok {
		return nil, ctx, fmt.Errorf(`no key "%v" in Secret %v`, encryptionKeyRef.Key, encryptionKeyRef.SecretName)
	}
