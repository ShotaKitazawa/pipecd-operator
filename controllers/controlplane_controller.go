...
func (r *ControlPlaneReconciler) reconcile(
	...
	controllerutil.AddFinalizer(patchCp, controlPlaneFinalizerName)
	// TODO: 複数 controller が 1object を操作する場合 server-side apply だと conflict する
	if err := r.Patch(ctx, patchCp, client.Apply, &client.PatchOptions{FieldManager: controlPlaneControllerName}); err != nil {
...
func (r *ControlPlaneReconciler) reconcileGeneral(
	...
	if _, err := ctrl.CreateOrUpdate(ctx, r, cm, func() (err error) {
		cm.Data, err = object.MakeConfigMapData(*cp)
		if err != nil {
			return err
		}
