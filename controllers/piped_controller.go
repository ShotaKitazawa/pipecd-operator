...
func (r *PipedReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	pipedService, ctx, err := services.NewPipedService(ctx, r.Client, req.Namespace, p.Spec.ProjectId, p.Spec.Insecure)
	if err != nil {
	...
	environmentService, ctx, err := services.NewEnvironmentService(ctx, r.Client, req.Namespace, p.Spec.ProjectId, p.Spec.Insecure)
	if err != nil {
...
func (r *PipedReconciler) reconcile(
	...
	controllerutil.AddFinalizer(patchPiped, pipedFinalizerName)
